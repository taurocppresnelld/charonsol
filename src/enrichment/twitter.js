...
// 2026-08-18: real logs showed the exact same dead tweet URL getting re-fetched 121, 52, 51, 45+
// times each — every one a fresh network call to a URL already confirmed 404. No caching existed
// at all here, unlike jupiterAssetCache elsewhere in the enrichment layer. 1h TTL: long enough
// that a dead URL isn't re-hit every single monitoring cycle, short enough that if fxtwitter
// itself has a transient blip, we're not stuck treating a real tweet as permanently dead.
const twitterCache = new Map(); // url -> { at, result }
const TWITTER_CACHE_TTL_MS = 60 * 60 * 1000;

async function fetchTwitterNarrative(graduatedCoin, gmgn) {
  const url = extractTweetUrl(graduatedCoin) || extractTweetUrl(gmgn);
  if (!url) return null;

  const cached = twitterCache.get(url);
  if (cached && now() - cached.at < TWITTER_CACHE_TTL_MS) return cached.result;
  ...
  try {
    ...
    twitterCache.set(url, { at: now(), result });
    return result;
  } catch (apiErr) {
  ...
  try {
    ...
    const result = { url, fxUrl: toFxTwitter(url), text: null, error: err.message };
    twitterCache.set(url, { at: now(), result });
    return result;
  }
}
